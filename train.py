...
if __name__ == "__main__":
    ...
    if i in list(range(15)):
        ...
        trainlabelpath = [os.path.join(path, j) for j in trains] 
        
        
        save_path = os.path.join(config["save"]["save_path"], f"checkpoint/{tests.split('.')[0]}")
        ...
        #                          num_workers=0)
        dataset = reader.txtload(trainlabelpath, root, config["params"]["batch_size"], shuffle=True,
                                num_workers=0)
