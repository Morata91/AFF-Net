import model
import reader.reader_mpii as reader
import numpy as np
import cv2 
import torch
...
if __name__ == "__main__":
    config = yaml.load(open(sys.argv[1]), Loader=yaml.SafeLoader)
    config = config["test"]
    path = os.path.join(config["data"]["path"], 'Label')
    root = os.path.join(config["data"]["path"], 'Image')
    ...
    if i in list(range(15)):
        ...
        testlabelpath = os.path.join(path, tests)

        save_path = os.path.join(config["load"]["load_path"], f"checkpoint/{tests.split('.')[0]}")

        save_name=f"evaluation/{tests.split('.')[0]}"
        if not os.path.exists(os.path.join(load_path, save_name)):
            os.makedirs(os.path.join(load_path, save_name))

        print("Read data")
        dataset = reader.txtload(testlabelpath, root, 256, num_workers=0, shuffle=False)
        ...
        all_log = open(os.path.join(load_path, f"evaluation/all.log"), 'a')
        all_log.write(f'Test Set:{tests}')
